...
struct RecurringEntry: TimelineEntry {
    ...
    static var placeholder: RecurringEntry {
        ...
        return RecurringEntry(date: Date(), data: WRecurringData(items: [
            .init(id: "1", name: "Netflix",   amount: 15.99, nextDate: d1,  frequencyLabel: "Monthly", assetName: "Checking", isInflow: false),
            .init(id: "2", name: "Salary",    amount: 3200,  nextDate: d7,  frequencyLabel: "Monthly", assetName: "Savings",  isInflow: true),
            .init(id: "3", name: "Gym",       amount: 40,    nextDate: d30, frequencyLabel: "Monthly", assetName: "Card",     isInflow: false),
            .init(id: "4", name: "Rent",      amount: 1200,  nextDate: d30, frequencyLabel: "Monthly", assetName: "Checking", isInflow: false),
            .init(id: "5", name: "Dividends", amount: 85,    nextDate: d30, frequencyLabel: "Monthly", assetName: "Stocks",   isInflow: true),
            .init(id: "6", name: "Dividends", amount: 85,    nextDate: d30, frequencyLabel: "Monthly", assetName: "Stocks",   isInflow: true),
...
struct RecurringWidget: Widget {
    ...
    var body: some WidgetConfiguration {
        ...
        .description("Upcoming recurring inflows and outflows.")
...
private struct RRow: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            Image(systemName: item.isInflow ? "arrow.up.circle.fill" : "arrow.down.circle.fill")
                .foregroundStyle(item.isInflow ? .green : .red).font(.subheadline)
            ...
            VStack(alignment: .trailing, spacing: 1) {
                Text(rFmt(item.amount, currency: currencyCode))
                    .font(.subheadline).fontWeight(.semibold).foregroundStyle(item.isInflow ? .green : .primary)
        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            if let item = entry.data.items.first {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    HStack(spacing: 4) {
                        Image(systemName: item.isInflow ? "arrow.up.circle.fill" : "arrow.down.circle.fill")
                            .foregroundStyle(item.isInflow ? .green : .red)
